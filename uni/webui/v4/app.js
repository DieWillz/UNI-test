...
function setBadge(cardSel, kind, text){
...
async function apiPost(path, body){
  return apiJson(path, {method:'POST', body: body?JSON.stringify(body):'{}'});
}
...
async function loadOverview(){
  ...
  try{
    ...
    setBadge('ov-cards'?.parentElement, 'ok', 'стек');
  }catch(e){
...
async function loadAgents(){
  ...
  try{
    ...
    const ags = d.agents||[];
    if(!ags.length){ box.innerHTML = badge('warn','НЕТ ДАННЫХ')+' — файлы heartbeat не найдены'; setBadge('ag-list'.replace('ag-list',''),'warn','нет данных'); return; }
    ...
    setBadge('ag-list'.replace('ag-list',''),'ok', ags.length+' аг.');
  }catch(e){ box.innerHTML = badge('err','ОШИБКА')+' '+esc(e.message); }
}
...
async function loadTasks(){
  try{
    ...
    setBadge('tk-backlog'.replace('tk-backlog',''),'ok', items.length+' пунктов');

    const ph = d.phases||[];
    $('tk-phases').innerHTML = ph.length ? ph.map(p=>`<div class="kv"><span>${esc(p.name||p.id||'фаза')}</span><span>${badge(p.done?'ok':'info', p.done?'готово':'в работе')}</span></div>`).join('') : badge('warn','НЕТ ДАННЫХ')+' — runtime/admin/phases.json пуст';
    setBadge('tk-phases'.replace('tk-phases',''), ph.length?'ok':'warn', ph.length?ph.length+' фаз':'нет фаз');
...
async function loadJournal(){
  ...
  try{
    ...
    if(d.error){ box.innerHTML = badge('warn','НЕТ ДАННЫХ')+' — '+esc(d.error); setBadge('jr-list'.replace('jr-list',''),'warn','нет данных'); return; }
    const ents = d.entries||[];
    if(!ents.length){ box.innerHTML = badge('warn','НЕТ ДАННЫХ')+' — журнал пуст'; setBadge('jr-list'.replace('jr-list',''),'warn','пусто'); return; }
    ...
    setBadge('jr-list'.replace('jr-list',''),'ok', ents.length+' зап.');
  }catch(e){ box.innerHTML = badge('err','ОШИБКА')+' '+esc(e.message); }
}
...
async function loadReports(){
  ...
  try{
    ...
    rpNames = (d.reports||[]);
    if(!rpNames.length){ list.innerHTML = badge('warn','НЕТ ДАННЫХ'); setBadge('rp-list'.replace('rp-list',''),'warn','нет'); return; }
    ...
    setBadge('rp-list'.replace('rp-list',''),'ok', rpNames.length+' шт.');
  }catch(e){ list.innerHTML = badge('err','ОШИБКА')+' '+esc(e.message); }
}
...
async function loadReportContent(name){
  ...
  try{
    ...
    setBadge('rp-content'.replace('rp-content',''),'ok','загружено');
...
const TOOLS = [
  ...
  {id:'pytest', name:'pytest (фон)', desc:'Фоновый прогон тестов, результат в runtime/pytest_last.json.', btn:'▶ Запустить', cls:'amber', run:async()=>apiPost('/api/admin/actions',{action:'run_pytest'})},
  {id:'arch', name:'Арх-аудит', desc:'py_compile всех uni/*.py (uni.check_architecture --strict).', btn:'▶ Проверить', cls:'amber', run:async()=>apiPost('/api/admin/actions',{action:'run_arch_check'})},
...
async function loadVision(){
  ...
  try{
    ...
    setBadge('vs-consent'.replace('vs-consent',''),'ok','consent');
...
async function loadAutonomous(){
  ...
  try{
    ...
    const a = d.autonomous||{};
    const on = a.enabled === true;
    const verif = d.verification_enabled === true;
    box.innerHTML =
      `<div class="kv"><span>autonomous.enabled</span><span>${badge(on?'ok':'warn', on?'ВКЛ':'ВЫКЛ')}</span></div>
    ...
    setBadge('au-status'.replace('au-status',''), on?'ok':'warn', on?'включен':'выключен');
  }catch(e){ box.innerHTML = badge('err','ОШИБКА')+' '+esc(e.message); }
}
...
async function loadXtoys(){
  ...
  try{
    ...
    setBadge('xt-cards'.replace('xt-cards',''),'ok','XToys');
  }catch(e){ cards.innerHTML = `<div class="tile">${badge('err','ОШИБКА')} ${esc(e.message)}</div>`; }
}
...
async function loadSettings(){
  try{
    ...
    const ports = [];
    if(c.webui) ports.push('8787');
    if(c.llm) ports.push('1235');
    $('st-config').textContent =
      'Публичные порты: '+(ports.join(', ')||'—')+'\n'+
      'council.provider: '+(c.council?.provider||'—')+'\n'+
      'vision.enabled: '+(c.vision?.enabled??'—')+'\n'+
      'autonomous.enabled: '+(c.autonomous?.enabled??'—')+'\n'+
      'verification_enabled: '+(c.verification_enabled??'—')+'\n\n'+
      '⚠ config.yaml с секретами не отдаётся (запрет API).';
    setBadge('st-config'.replace('st-config',''),'ok','read-only');

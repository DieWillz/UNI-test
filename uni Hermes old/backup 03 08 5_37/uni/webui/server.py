...
# UNI-UI-002 §3: cache of rate-limit detection per browser participant.
# Populated during a round (when a live BrowserSession exists) and surfaced
# by /api/status without spinning up a second CDP connection. Values:
# {name: {"limit_exceeded": bool, "reset_in": str | None}}
...
# Live agent state for the WebUI status widget. Mutated from the SSE round worker
# and read by GET /api/status; a plain dict is enough since the HTTP server is
# single-threaded per request and only ever replaces whole values.
_AGENT_STATUS: dict[str, Any] = {"state": "idle", "last_action": "", "round_id": None, "updated": None}


def _set_status(state: str, last_action: str = "", round_id: str | None = None) -> None:
    import time

    _AGENT_STATUS["state"] = state
    if last_action:
        _AGENT_STATUS["last_action"] = last_action
    if round_id is not None:
        _AGENT_STATUS["round_id"] = round_id
    _AGENT_STATUS["updated"] = time.strftime("%H:%M:%S")
...
def _tcp_open(host: str | None, port: int, timeout: float = 0.6) -> bool:
    """True when a TCP connect to host:port succeeds (local server liveness check)."""
    if not host:
        return False
    import socket
...
def _dns_ok(host: str | None, timeout: float = 1.5) -> bool:
    """True when the hostname resolves. Distinguishes 'DNS dead' from 'bad key'."""
    if not host:
        return False
    import socket
...
def _participant_statuses(cfg) -> list[dict[str, Any]]:
    ...
    for participant in load_participants(min_interval_seconds=cfg.council.min_interval_seconds):
        ...
        if participant.transport == "browser":
            ...
            if not base_url:
                ...
                if _tcp_open(host, port):
                    status, detail = "ready", f"локальный сервер отвечает ({host}:{port})"
                else:
                    status, detail = "unavailable", (
                        f"локальный сервер не запущен ({host}:{port}) — "
                        f"запустите LM Studio / локальный сервер или отключите участника"
                    )
            elif not _dns_ok(host):
                status, detail = "unavailable", f"DNS/сеть: имя {host} не разрешается (это не ключ)"
            else:
        ...
        statuses.append({
            ...
            "status": status,
            "detail": detail,
            "limit_exceeded": bool(_LIMIT_CACHE.get(participant.name, {}).get("limit_exceeded", False)),
            "limit_reset": _LIMIT_CACHE.get(participant.name, {}).get("reset_in"),
        })
    return statuses


async def _detect_limits(session, selected) -> None:
    """UNI-UI-002 §3: scan each browser participant's open tab for a rate-limit banner.

    Results land in the module-level _LIMIT_CACHE consumed by /api/status.
    Failures are swallowed per-tab (Edge Case 3): a single closed tab never
    blocks the round.
    """
    ...
    for p in selected:
        if p.transport != "browser":
            continue
        host = (p.spec.get("host") or "") if hasattr(p, "spec") else ""
        try:
            page = await session.page_for_host(host) if host else None
        except Exception:
            page = None
        if page is None:
            _LIMIT_CACHE.pop(p.name, None)
            continue
        try:
            res = await session.check_limit_banner(page)
        except Exception:
            continue
        _LIMIT_CACHE[p.name] = {
            "limit_exceeded": bool(res.get("limit_exceeded")),
            "reset_in": res.get("reset_in"),
        }
...
async def run_round(payload: dict, emit) -> dict:
    ...
    # UNI-UI-002 §3: probe browser tabs for rate-limit banners before the round.
    if session is not None:
        await _detect_limits(session, selected)
    try:
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/favicon.ico":
            # 1x1 transparent GIF — silences the 404 noise in the browser console.
            self._send(200, base64.b64decode(
                "R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"
            ), "image/gif")
            return
        if parsed.path == "/api/status":
            # Two-level payload consumed by the v2.7 compact UI:
            #   - participants[]: live CDP/tab/state per council member (auto-polled)
            #   - agent: the overall runner state (idle/running/error) set during a round
            cfg = load_config()
            participants = _participant_statuses(cfg)
            out = {
                "participants": [
                    {
                        "name": p["name"],
                        "type": p["transport"],
                        "cdp_ok": p["transport"] == "browser" and p["status"] == "ready",
                        "tab_found": p["transport"] == "browser" and p["status"] == "ready",
                        "state": p["status"],
                        "detail": p.get("detail", ""),
                        "limit_exceeded": bool(p.get("limit_exceeded", False)),
                        "limit_reset": p.get("limit_reset"),
                    }
                    for p in participants
                ],
                "agent": dict(_AGENT_STATUS),
            }
            self._json(200, out)
            return
        ...
        if parsed.path == "/api/history":
            ...
            return
        if parsed.path in ("/api/report", "/api/report/raw"):
            cfg = load_config()
            ...
            if parsed.path == "/api/report/raw":
                # Served as plain text so the report file opens directly in a browser tab.
            ...
            self._json(200, {
                ...
                "raw_url": f"/api/report/raw?id={round_id}",
            })
            return
        ...
        if self.path == "/api/round/start":
            ...
            async def _stream():
                ...
                async def emit(event: dict):
                    # Mirror round progress into the status widget feed.
                    etype = event.get("type")
                    if etype == "stage":
                        _set_status("running", str(event.get("name") or event.get("msg") or "работа"))
                    elif etype == "answer":
                        _set_status("running", f"ответ: {event.get('name', '?')}")
                    elif etype == "done":
                        _set_status("idle", "раунд завершён", event.get("round_id"))
                    await queue.put(event)

                async def worker():
                    try:
                        _set_status("running", "раунд запущен")
                        await run_round(payload, emit)
                        _set_status("idle", "раунд завершён")
                    except Exception as exc:  # never kill the SSE socket on a round error
                        _set_status("error", f"{type(exc).__name__}: {exc}")

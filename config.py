...
def load_config(path: str = "config.yaml") -> Config:
    ...
    """
    # Сначала пробуем путь как есть (относительно CWD)
    cfg_path = Path(path)
    
    # Если не найден — ищем относительно корня проекта (где лежит uni/__main__.py)
    ...
    if not cfg_path.exists():
        raise FileNotFoundError(
            ...
            f"Скопируйте config.yaml в корень проекта или укажите верный путь."
        )

...
let HRM=localStorage.getItem('uni_hrm')||'http://127.0.0.1:8790';
if(HRM==='http://127.0.0.1:8787'){HRM='http://127.0.0.1:8790';localStorage.setItem('uni_hrm',HRM)}
...
<div class="xt-head"><div><b>🔌 XToys · Fredorch Rotary</b> <span class="pill p-warn" id="xStatus">ожидание Intiface</span></div><button class="tbtn danger" onclick="xtoysEmergencyStop()">🛑 Аварийный стоп</button></div>
<div class="xt-stats" id="toyControlStatus">
 ...
 <div class="stat-card"><div class="stat-info"><h4>Безопасность</h4><h2 id="toyEmergencyState">норма</h2><p>fail-safe включён</p></div></div>
</div>
<div class="xt-dashboard">
 <section class="card xt-card"><div class="ch">🎚 Управление Юни</div><div class="cb">
  ...
  <div class="x-grid"><button class="x-btn" onclick="xtoysPattern('tease')">Tease</button><button class="x-btn" onclick="xtoysPattern('build')">Build</button><button class="x-btn" onclick="xtoysPattern('pulse')">Pulse</button><button class="x-btn" onclick="xtoysPattern('wave')">Wave</button><button class="x-btn" onclick="xtoysPattern('edge')">Edge</button><button class="x-btn" onclick="xtoysPattern('release')">Release</button></div>
 ...
 <section class="card xt-card"><div class="ch">🎞 Motion → машинка</div><div class="cb">
  ...
  <div class="xt-actions"><button class="tbtn" onclick="motionCameraStart()">Превью</button><button class="tbtn" onclick="motionCameraStop()">Выключить</button><button class="tbtn" onclick="motionCameraSnap()">Кадр Юни</button></div><div id="motionCameraPreview" class="xt-camera"><div>камера выключена</div></div>
 </div></section>
 <section class="card xt-card"><div class="ch">🔌 Intiface и доступ</div><div class="cb">
  ...
  <div class="xt-actions"><input id="remoteUrl" class="tin xt-url" readonly placeholder="ссылка появится здесь"><button class="tbtn" onclick="remoteCopy()">Копировать</button></div><div class="xt-section danger-text">🛡 Безопасность</div>
  <div class="xt-actions"><button class="tbtn danger" onclick="xtoysEmergencyStop()">🛑 Emergency Stop</button><button class="tbtn" onclick="xtoysEmergencyReset()">Сбросить стоп</button></div>
 </div></section>
</div>`}
...
async function motionCameraStart(){try{motionCameraStop();_motionCameraStream=await navigator.mediaDevices.getUserMedia({video:true});const v=document.createElement('video');v.srcObject=_motionCameraStream;v.autoplay=true;v.muted=true;v.style.cssText='width:100%;height:100%;object-fit:contain';const p=$('motionCameraPreview');p.innerHTML='';p.appendChild(v);p.style.display='block'}catch(e){showToast('⚠ Камера: '+e.message)}}
...
async function motionCameraSnap(){try{await xtPost('/api/camera/start',{notice_ack:true});const r=await xtPost('/api/vision/capture');if(!r.image_b64)throw new Error('кадр не получен');motionCameraStop();const img=new Image();img.src=r.image_b64;img.style.cssText='width:100%;height:100%;object-fit:contain';const p=$('motionCameraPreview');p.innerHTML='';p.appendChild(img);p.style.display='block'}catch(e){showToast('⚠ Кадр Юни: '+e.message)}}
...
function remoteStart(){xtPost('/api/xtoys/remote/session/start',{max_intensity:Number($('remoteMax').value),ttl:Number($('remoteMinutes').value)*60}).then(d=>{$('remoteUrl').value=d.url;remotePoll();showToast('Удалённая сессия создана')}).catch(e=>showToast('⚠ Remote: '+e.message))}
function remoteStop(){xtPost('/api/xtoys/remote/session/stop').then(()=>{$('remoteUrl').value='';remotePoll()}).catch(e=>showToast('⚠ Remote stop: '+e.message))}
...
function xtoysEmergencyStop(){xtPost('/api/xtoys/emergency-stop').then(()=>showToast('🛑 Машинка аварийно остановлена')).catch(e=>showToast('⚠ '+e.message))}
function xtoysEmergencyReset(){xtPost('/api/xtoys/emergency-reset').then(()=>showToast('Аварийный стоп сброшен')).catch(e=>showToast('⚠ '+e.message))}

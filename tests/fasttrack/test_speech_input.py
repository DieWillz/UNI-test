...
class SpeechInputTests(unittest.TestCase):
    ...
    def test_voice_activated_recording_stops_after_trailing_pause(self):
        ...
        chunks = [silence, voice.clip(-1, 1)]
        # Build a controllable stream that emits: silence, voice, voice, silence, silence.
        stream_chunks = [silence, voice, voice, silence, silence]
        ...
        class FakeStream:
            ...
            def read(self, _blocksize):
                return stream_chunks.pop(0), False

        # 🤖 Patch-point: sounddevice is lazy-loaded into the module global
        # ``sd`` (see speech._ensure_sd). The test injects a stub module so the
        # recording path runs without a real audio device. This still exercises
        # the real trailing-pause stop logic (not mocked away).
        class FakeSD:
            InputStream = FakeStream

        with patch("uni.capabilities.speech.sd", FakeSD()):

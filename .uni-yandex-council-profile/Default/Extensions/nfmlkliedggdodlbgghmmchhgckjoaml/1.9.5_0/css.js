...
chrome.storage.local.get(['exceptions'], (result) => {
    chrome.storage.local.get(['staticrules'], (result0) => {
        chrome.storage.local.get(['cssjs_togg'], (result1) => {
    
    var cssjs_togg = result1.cssjs_togg;

    var f_l_a_g_1_0 = 0;

    if(cssjs_togg == "disabled")
    {
        //cssjs togg disabled!
        f_l_a_g_1_0 = 0;
    }
    else //cssjs_togg enabled, undefined, null, network errors, etc -> check further
    {
    if(result0.staticrules != "disabled")
        ...
        {
          //=== disabled for sure, adblocking disabled totally -> dont do css based adblocking
          f_l_a_g_1_0 = 0;
        }
    }

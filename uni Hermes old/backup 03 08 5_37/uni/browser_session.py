...
class BrowserSession:
    ...
    async def close(self) -> None:
        ...
        if context is not None and owns_context:
            ...
            # Attached over CDP: detach without killing the user's own browser.
            try:
    ...
    async def start_keep_alive(self, interval_seconds: float = 30.0) -> None:
        """Ping the page periodically so an idle CDP socket is not dropped.

        A backgrounded tab/websocket can be reaped after minutes of inactivity; a
        trivial ``1+1`` evaluation keeps the channel warm. Failures are swallowed —
        the next real action goes through ``ensure_alive`` and reconnects anyway.
        """
        ...
        async def _loop() -> None:
            while True:
                ...
                try:
                    ...
                    pass  # ensure_alive() will heal it on the next real action
    ...
    async def ensure_alive(self) -> None:
        ...
        For a CDP attachment this reconnects to the same debugging endpoint rather
        than launching a second browser, so the user's window is reused.
        """
    ...
    # --- Rate-limit banner detection (UNI-UI-002, §3) ---------------------------
    _LIMIT_PATTERNS = [
        re.compile(
            r"(\d+)\s*(?:час[а-я]+)?\s*(\d+)\s*(?:минут[а-я]+)?\s*до\s*сброса\s*лимита",
            re.IGNORECASE,
        ),
        re.compile(r"до\s*сброса\s*лимита", re.IGNORECASE),
        re.compile(r"rate\s*limit", re.IGNORECASE),
        re.compile(r"supergrok", re.IGNORECASE),
        re.compile(r"превыш[а-я]+\s*лимит", re.IGNORECASE),
    ]

    async def check_limit_banner(self, page: "Page") -> dict[str, Any]:
        """Detect a rate-limit / quota banner inside a participant's browser tab.

        Returns {"limit_exceeded": bool, "reset_in": "16ч 11мин" | None}.

        Must never raise and must not block the event loop: every DOM read is
        bounded by a 2s timeout (Edge Case 3 of UNI-UI-002). On any failure
        (CDP drop, navigation, timeout) we report a clean "unknown" so callers
        can keep their last known good state.
        """
        result: dict[str, Any] = {"limit_exceeded": False, "reset_in": None}
        if page is None:
            return result
        try:
                ...
                page.evaluate(
                    """() => {
                        ...
                        document.querySelectorAll('body *').forEach(el => {
                            if (vis(el) && el.children.length <= 3) {
            ...
            # CDP drop / timeout / navigation: keep last known state.
            return result

        for pat in self._LIMIT_PATTERNS:
            m = pat.search(text)
            if m:
                result["limit_exceeded"] = True
                if pat.groups >= 2 and m.groups() >= 2 and m.group(1) and m.group(2):
                    result["reset_in"] = f"{m.group(1)}ч {m.group(2)}мин"
                break
        return result

...
// 🤖 Единая точка входа (2026-08-13, §2/§3/§4): Самотест, Демо мыши, выбор интерфейса
$("btnSelfTest").onclick = () => $("selfTest").classList.toggle("hidden");
$("btnRunSelfTest").onclick = async () => {
  const body = $("selfTestBody");
  body.innerHTML = "⏳ выполняю самотест…";
  try {
    const r = await api("/api/selftest", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ save: true }) });
    const d = await r.json();
    if (d && d.ok && d.markdown) {
      const checks = d.checks || [];
      body.innerHTML = checks.map((c) => {
        const ic = c.status === "ok" ? "✅" : c.status === "skip" ? "⚠️" : "❌";
        const tip = c.hint ? `<div class="tip">${c.hint}</div>` : "";
        return `<div class="st ${c.status}">${ic} <b>${c.title}</b><div>${c.detail}</div>${tip}</div>`;
      }).join("");
    } else {
      body.innerHTML = "⚠ " + ((d && d.error) || "ошибка самотеста");
    }
  } catch (e) { body.innerHTML = "⚠ " + e.message; }
};
$("btnMouseDemo").onclick = async () => {
  showBubble("🖱 Юни ведёт мышь…");
  try {
    const r = await api("/api/demo/mouse", { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" });
    const d = await r.json();
    showBubble(d && d.ok ? "✅ Демо мыши выполнено (кольцо «Юни»)" : "⚠ " + ((d && d.error) || "нет мыши"));
  } catch (e) { showBubble("⚠ " + e.message); }
};
...
async function loadSettings() {
  ...
  if (U) { const st = await U.loadState(); if (st) { $("autostartChk").checked = !!st.autostart; $("opacity").value = st.opacity || 1; if (st.ui_variant) $("variantSel").value = st.ui_variant; } }
}
$("saveSettings").onclick = async () => {
  ...
  // 🤖 §4: сохранить выбор интерфейса (classic/v4) через IPC -> state.json; применится при reload
  const variant = $("variantSel").value;
  if (U && U.saveState) { U.saveState({ autostart, opacity, ui_variant: variant }); }
  if (U && U.setUiVariant) { try { await U.setUiVariant(variant); } catch (e) {} }
  if (U) U.setOpacity(opacity);
  showBubble("✅ настройки сохранены" + (variant === "v4" ? " (v4 при след. запуске)" : ""));
};

"""
tts_sentence_chunker.py — АДАПТИРОВАННАЯ ВЫЖИМКА из Hermes tools/tts_streaming.py
для проекта Юни (C:\\LLM\\UNI\\uni).

Источник: hermes-agent/tools/tts_streaming.py (488 строк, © Hermes Agent).
Взято: SentenceChunker (режет поток LLM-токенов на предложения для
построчной озвучки) + interruption latch (прерывание речи при бардже).

ЗАЧЕМ ЮНИ: сейчас TTS в uni/capabilities/speech.py (Piper/Silero) ждёт весь
ответ, потом озвучивает. Этот паттерн позволяет говорить ПО ПРЕДЛОЖЕНИЯМ
(как Hermes), не дожидаясь конца, и корректно прерывать речь, если
пользователь перебил.

Очищено от зависимостей Hermes (tools.tts_tool, openai, elevenlabs и т.д.).
Только логика нарезки + latch. Провайдеры TTS — задача Юни (Piper/Silero).
"""

...
class SpeechSynthesisTests(unittest.TestCase):
    ...
    def test_escape_interrupts_chunked_playback(self):
        ...
        # ``sd`` (see speech._ensure_sd). Inject a stub module so playback runs
        # without a real audio device; the real chunking + escape-interrupt
        # logic is still exercised (not mocked away).
        class FakeSD:
            OutputStream = FakeOutputStream

        with patch("uni.capabilities.speech.sd", FakeSD()), patch.object(
            speech,
            "_escape_pressed",

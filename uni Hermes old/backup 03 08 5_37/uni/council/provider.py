...
class FallbackProvider(CouncilProvider):
    """Cascading wrapper: tries providers in order until one returns a usable reply.

    Contract is unchanged — it *is* a CouncilProvider and returns a plain
    ParticipantReply, so round.py / run.py need no changes. A reply counts as a
    failure when ``error`` is set or the text came back empty; in that case the
    next provider in the chain is tried (403 key limit, 404 missing model, DNS
    failures and timeouts are all covered by that rule).

    If every provider fails, the aggregated per-step reasons are returned as a
    single structured error so the UI can show *why* the whole chain died.
    """

    scheme = "fallback"

    def __init__(self, steps: list[tuple[str, CouncilProvider]]) -> None:
        if not steps:
            raise ValueError("FallbackProvider requires at least one provider")
        self.steps = steps

    @property
    def model(self) -> Optional[str]:  # surfaced in reports like a plain provider
        return getattr(self.steps[0][1], "model", None)

    async def ask(self, participant: str, prompt: str, *, max_tokens: int = 2000) -> ParticipantReply:
        import logging
        import time

        log = logging.getLogger("uni.council.fallback")
        start = time.perf_counter()
        failures: list[str] = []
        for index, (label, provider) in enumerate(self.steps):
            try:
                reply = await provider.ask(participant, prompt, max_tokens=max_tokens)
            except Exception as exc:  # a provider may raise instead of reporting
                reason = f"{type(exc).__name__}: {exc}"
                failures.append(f"{label}: {reason}")
                log.warning("Council fallback: %s via %s failed (%s)", participant, label, reason)
                continue
            if reply.error is None and reply.text.strip():
                if index:  # note which backup actually answered
                    log.info("Council fallback: %s answered via backup %s", participant, label)
                return reply
            reason = reply.error or "empty response"
            failures.append(f"{label}: {reason}")
            log.warning("Council fallback: %s via %s failed (%s)", participant, label, reason)
        return ParticipantReply(
            participant=participant,
            text="",
            via=self.steps[0][1].scheme,
            model=self.model,
...
def build_provider(spec: dict[str, Any]) -> CouncilProvider:
    ...
    When the spec carries a non-empty ``fallbacks`` list (each item being a spec of
    the same shape), the primary provider is wrapped in a :class:`FallbackProvider`
    so a dead key/model/endpoint transparently rolls over to the next one.
    """
    fallbacks = spec.get("fallbacks") or []
    if fallbacks:
        base = dict(spec)
        base.pop("fallbacks", None)
        steps: list[tuple[str, CouncilProvider]] = [
            (str(base.get("endpoint") or base.get("model") or base.get("transport", "primary")),
             build_provider(base))
        ]
        for item in fallbacks:
            if not isinstance(item, dict):
                continue
            child = dict(item)
            child.pop("fallbacks", None)
            label = str(child.get("endpoint") or child.get("model") or child.get("transport", "backup"))
            try:
                steps.append((label, build_provider(child)))
            except Exception:  # a malformed backup must never break the primary
                continue
        return FallbackProvider(steps)

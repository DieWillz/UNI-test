...
function showView(v,el){document.querySelectorAll('.view-wrap').forEach(x=>x.classList.remove('active'));document.querySelectorAll('.nav-item').forEach(x=>x.classList.remove('active'));const w=$('view-'+v);if(w)w.classList.add('active');if(el)el.classList.add('active');if(v==='xtoys'){initXtoysExtras();initMotionCameraUI();initToyControlStatus();upgradeToyControlStatus();xtoysSessionPoll();intifacePoll();xtoysPatternPoll()}}
...
function motionCameraStop(){if(_motionCameraStream){_motionCameraStream.getTracks().forEach(t=>t.stop());_motionCameraStream=null}const p=$('motionCameraPreview');if(p){p.innerHTML='';p.style.display='none'}}

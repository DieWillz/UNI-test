...
def _fallback_chain(endpoint: str, cfg: Any) -> list[str]:
    """Backup endpoint names for ``endpoint`` from ``council.fallbacks``.

    Config shape (all optional)::

        council:
          fallbacks:
            openrouter: [groq, hermes]

    Returns an empty list when nothing is configured, so behaviour is unchanged
    for users who never opt in.
    """
    council = getattr(cfg, "council", None) if cfg is not None else None
    table = getattr(council, "fallbacks", None) or {}
    if not isinstance(table, dict):
        return []
    chain = table.get(endpoint) or []
    if isinstance(chain, str):
        chain = [chain]
    return [str(x) for x in chain if x and str(x) != endpoint]
    ...
    *,
    ...
    for spec in specs:
        ...
        if clean.get("transport") == "api" and clean.get("endpoint"):
            ...
            # Optional resilience chain: council.fallbacks.<endpoint> = [names...].
            # A dead key/model (403/404) then rolls over to the next live endpoint
            # instead of dropping the participant from the round.
            for backup in _fallback_chain(clean["endpoint"], cfg):
                bep = resolve_endpoint(backup, cfg)
                if not bep or not bep.get("base_url"):
                    continue
                clean.setdefault("fallbacks", []).append({
                    "transport": "api",
                    "endpoint": backup,
                    "base_url": bep["base_url"],
                    "api_key": bep["api_key"],
                    "model": bep.get("model") or clean.get("model"),
                    "timeout_seconds": clean.get("timeout_seconds", 60.0),
        ...
        display_spec = {k: v for k, v in clean.items() if k not in ("api_key", "fallbacks")}
        if clean.get("fallbacks"):
            # Never expose backup api_keys; only the chain order is public.
            display_spec["_fallbacks"] = [f.get("endpoint") for f in clean["fallbacks"]]
        if "api_key" in clean:

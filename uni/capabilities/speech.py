...
# 🤖 Ground-truth (B-группа): ленивые модульные patch-пойнты для тестов.
# НЕ модульный import — не ломает A-05 (import uni.capabilities не падает
# при отсутствии PortAudio/faster-whisper/piper). Заполняются при первом
# реальном использовании; тесты патчат uni.capabilities.speech.WhisperModel
# и uni.capabilities.speech.sd до вызова.
WhisperModel = None
sd = None


def _ensure_whisper_cls():
    global WhisperModel
...
class SpeechCapability(Capability):
    ...
    def _resolve_voice(value: str) -> str:
        ...
        # Conventional locations for Piper/voice model files (local-first).
        # 🤖 FIX (Hermes, 2026-08-12): added downloads/ and uni/assets/voices/
        # so a voice dropped in either place resolves regardless of cwd.
        search_roots = [
            Path.cwd(),
            project_root,
            project_root / "downloads",
            project_root / "uni" / "assets" / "voices",
        ]
        names = [candidate]
        if candidate.suffix != ".onnx":
            names.append(candidate.with_suffix(".onnx"))
    ...
    async def _init_stt(self) -> None:
        if self._whisper is None:
            async with self._stt_lock:
                if self._whisper is None:
                    whisper_cls = _ensure_whisper_cls()

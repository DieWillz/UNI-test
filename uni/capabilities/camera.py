...
class CameraCapability(Capability):
    ...
    async def start(self, notice_ack: bool = False) -> ToolResult:
        ...
        # включается по прямому вызову. notice_ack оставлен для обратной
        # совместимости (исторические вызывающие могут передавать True),
        # но он больше не блокирует запуск.
        try:
